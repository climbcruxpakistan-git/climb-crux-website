/**
 * Order payment-received email — the FIRST customer email in the equipment
 * order flow. Sent only after the customer uploads their payment screenshot
 * (never on order placement). Confirms the payment is under verification.
 */
 ...
 */
export function orderPaymentReceivedEmail({ order, whatsapp = '' }) {
  ...
  const html = renderEmailLayout({
    headerTitle: 'Payment Received — Under Verification',
    headerSubtitle: 'Your payment is being reviewed by our team',
    greeting: `Hi ${escapeHtml((order.customer_name || 'there').split(' ')[0])},`,
    bodyHtml: `
      <p style="margin:0 0 6px;font-size:14px;color:#444;line-height:1.7">
        Thank you for your equipment order! We have <strong>received your
        payment</strong> and it is now <strong>under verification</strong> by
        the Climb Crux team. You will receive a confirmation email as soon as
      ...
      <p style="margin:18px 0 0;font-size:14px;color:#444;line-height:1.7">
        A <strong>PDF copy of your order</strong> has been attached to this
